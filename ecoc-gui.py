...
def main_app():
    ...
    def fetch_vegvesen_data(file_path, iviref_uid):
        ...
        # test env
        url = "https://synt.utv.vegvesen.no/ws/no/vegvesen/kjoretoy/felles/innmelding/meldingompreregistrering/v1/opprette"
        # prod env
        ...
        if response.status_code == 200:
            ...
            understellsnummer = response_dict.get("iviIdentifikator", {}).get(
                "understellsnummerMerke", {}).get("understellsnummer", "")
    ...
    def read_vehicle_identification_number(file_path):
        try:
            tree = ET.parse(file_path, parser=ET.XMLParser(encoding='utf-8'))
            root = tree.getroot()
            # Leser fra XML, må være korrekt tag.
    ...
    def save_settings_to_db():
        ...
        if not audience:
            # Default audience (Produksjonsmiljø)
        ...
        if not keystore_alias_password:
            # Default keystore alias password
        ...
        try:
            ...
            c.execute("INSERT INTO samarbeidsportalen (issuer, audience, resource, scope, keystore_password, keystore_alias, keystore_alias_password) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    keystore_password_entry = add_labeled_entry(
        samarbeidsportalen_frame, "Keystore Password:")
    keystore_alias_entry = add_labeled_entry(
        samarbeidsportalen_frame, "Keystore Alias:")
    keystore_alias_password_entry = add_labeled_entry(
        samarbeidsportalen_frame, "Keystore Alias Password:")

    # Button to save settings
    save_button = ttk.Button(samarbeidsportalen_frame,
                             text="Save Settings", command=save_settings_to_db)
    ...
    generate_button = ttk.Button(
        frame1, text="Generer", command=generate_ivi_ref_id, bootstyle='info')
    ...
    def validate_input(char, value_if_allowed):
        if char in '0123456789' or value_if_allowed == '':
            try:
                ...
                logging.error(
                    f"An error occurred validating input: {ValueError}")
                return False
        else:
            return False
    ...
    # Table Treeview
    table = ttk.Treeview(table_and_search_frame, height=30, columns=("iviReferanse", "understellsnummer",
                         "datoTid", "meldingstekst"), yscrollcommand=table_scrollbar.set, show='headings')
    # Make sure to expand and fill
    ...
    # Set the width to 600 or any other value that suits your needs
    ...
    search_button = ttk.Button(search_frame, bootstyle="warning",
                               text="Søk", command=lambda: search_table(search_entry.get()))
    ...
    response_text = tk.Text(response_and_ividoc_frame, wrap=tk.WORD, width=50,
                            height=20, state=tk.DISABLED)  # increased height from 10 to 20
            ...
            if all(row):
                try:
                    ...
                    formatted_date = datetime.fromisoformat(
                        date_str.replace('T', ' ')).strftime("%Y-%m-%d")
    ...
    def on_table_select(event):
        try:
            ...
            c = conn.cursor()
            c.execute(
                "SELECT ividoc FROM responses WHERE understellsnummer = ?", (understellsnummer,))
    ...
    def delete_entry():
        ...
        # test env
        url = f"https://synt.utv.vegvesen.no/ws/no/vegvesen/kjoretoy/felles/innmelding/meldingompreregistrering/v1/slette/understellsnummer/{vin_to_delete}"
        # prod env
        ...
        pretty_server_response = json.dumps(
            json.loads(server_response), indent=4)

        if response.status_code == 200:
            conn = sqlite3.connect('vegvesen_data.db')
            c = conn.cursor()
            c.execute(
                "DELETE FROM responses WHERE understellsnummer = ?", (vin_to_delete,))
            conn.commit()
            conn.close()
            ...
            set_response_text(
                f"Failed to delete entry with VIN: {vin_to_delete}. HTTP Status Code: {response.status_code}\nServer Response: {pretty_server_response}")
    ...
    def run_pubkeygen():
        ...
        # Save public Key to a file if needed
    ...
    jwt_button = ttk.Button(
        frame3_tab3, text="Generate JWT Key", command=run_pubkeygen, bootstyle='info')
